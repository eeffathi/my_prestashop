 ...
 * $test = new cron_parser('1 3 2-8/6 * 2,3', '27-05-2013 23:00');
 * for($i = 0; $i < 3; $i++) {
 ...
 * $test = new cron_parser(array('1 3 2-8/6 * 2,3', '0 0 15 * *'), '27-05-2013 23:00');
 * for($i = 0; $i < 3; $i++) {
 *   echo date('d-m-Y H:i (w)', $test->get($i)).'<br />';
 * }</code>
 * will output:<pre>
 ...
 * echo date('d-m-Y H:i (w)', cron_parser::calc('1 3 2-8/6 * 2,3')).'<br />';
 * // all occurences in a year
 * $test = new cron_parser(array('0 0 15 * *'), '27-05-2013 23:00');
 ...
 * For more info see (
 *
 * @link       http://en.wikipedia.org/wiki/Cron the Wikipedia article on crons) or Google.
 *
 * @name       cron_parser
...
class csd_parser {
    ...
    public function get($which = 'next', $time = null) {
        ...
        if(!is_int($which)) {
            if(is_string($which)) $till = preg_match('/^\d+$/', $which) ? $which * 1 : strtotime($which);
            if(!isset($till) || empty($till)) {
                $trace = debug_backtrace();
                trigger_error(__METHOD__.": Incorrect value given for \$which in {$trace[0]['file']} on line {$trace[0]['line']}. . Using \"next\" instead.", E_USER_WARNING);
                $which = 0;
            }
        }
        if($time === null) $time = $this->base_time;
        else $time = self::parse_time($time);
        ...
        if(isset($till)) {
            ...
            do {
                if($store && isset($this->known[$index])) {
                    ...
                    if($time === null) break;
                    if($store) $this->known[$index] = $time;
                }
                $times[] = $time;
            ...
            for($i = 1; $i <= $runs; $i++) {
                if($store && isset($this->known[$up ? $i - 1 : -$i])) {
                    ...
                    $time = self::find_time($this, $time, $up);
                    if($time === null) break;
                    if($store) $this->known[$up ? $i - 1 : -$i] = $time;
                }
                if($i !== $runs) $time = $up ? $time + 60 : $time - 60;
            }
        }
        return $time;
    }
    ...
    static protected function find_time(csd_parser $parser, $time, $up = true) {
        ...
        foreach($parser->times as $time_data) {
            ...
            if($option !== null) {
                if($next === null) $next = $option;
                elseif($up) {
                    if($next > $option) $next = $option;
                } else {
                    if($next < $option) $next = $option;
                }
            }
    ...
    static protected function find_time_l1($time_data, $time, $up = true) {
        if(is_array($time)) $time = mktime($time[1], $time[0], 0, $time[3], $time[2], $time[5]);
        $rel_times = explode(' ', date('i H d m w Y', $time));
        foreach($rel_times as $key => $val) $rel_times[$key] = (int)$val;
        $with  = $rel_times;
        $limit = 100;
        if(!self::find_time_l2($time_data, $with, 5, $up)) return null;
        if(!self::find_time_l2($time_data, $with, 3, $up)) return null;
        do {
            if(!self::find_time_l2($time_data, $with, 2, $up)) return null;
            if(!self::find_time_l2($time_data, $with, 1, $up)) return null;
            if(!self::find_time_l2($time_data, $with, 0, $up)) return null;
            ...
            if(self::find_time_l2($time_data, $with, 4, $up)) {
                ...
                if($up) {
                    $offset = $time_data[4]->index[0] + 7 - $weekday;
                } else {
                    $offset = $weekday - $with[4];
                }
            }
            $with[2] += $offset;
    ...
    static protected function find_time_l2($for, &$with, $field_id, $up = true, $force = false) {
        if($field_id === 5 && !isset($for[$field_id])) return true; # years are optional
                ...
                if($value >= $curr) {
                    if($field_id !== 2 || # not a day,
                      ...
                      (self::ly($with[5]))
                        ...
                        $spill = false;
                        if($value === $curr && !$force) return true;
                        else break;
                    }
                }
            }
        ...
        for($i = $field_id - 1; $i >= 0; $i--) {
            switch($i) {
                ...
                case 2:
                    ...
                    if(self::$long[$with[3]]) $with[2] = 31; # long month: 31
                    elseif($with[3] !== 2) $with[2] = 30;
                    elseif(self::ly($with[5])) $with[2] = 29; # feb in a leapyear: 29
                    else $with[2] = 28;
                    break;
                case 1:
                    $with[1] = $up ? 0 : 23;
        ...
        switch($field_id) {
            ...
            case 2:
                if($up) {
                    ...
                    if(self::$long[$with[3] - 1]) $want = 31;
                    elseif($with[3] !== 3) $want = 30;
                    elseif(self::ly($with[5])) $want = 29;
                    else $want = 28;
                    return false;
                }
                break;
    ...
    static protected function prep_csds($csds) {
        ...
        foreach($csds as $line => $csd) {
            ...
            if(!isset($fields[4]) || isset($fields[6])) {
                $trace = debug_backtrace();
                trigger_error("{$trace[1]['function']}: Skipping incorrect definition '".implode(' ', $fields)."' found on line $line of the definitions provided in {$trace[1]['file']} on line {$trace[1]['line']}.", E_USER_WARNING);
                continue;
            }
            ...
            foreach($fields as $field_id => $field) {
                ...
                if(isset(self::$mapping[$type])) { # if we have a mapping replace according to it
                    $trace = debug_backtrace();
                    $field = str_ireplace(array_keys(self::$mapping[$type]), array_values(self::$mapping[$type]), $field);
                }
                if($field_id === 4) {
                    $old   = $field;
                    ...
                    if($old !== $field) {
                        ...
                        trigger_error("{$trace[1]['function']}: Use of incorrect value for (and/or place of) 7, 'sun' or 'sunday' in the definition-field value for $type ('".$fields[$field_id]."' in definition '".implode(' ', $fields)."') on line $line of the definitions provided in {$trace[1]['file']} on line {$trace[1]['line']}.", E_USER_NOTICE);
                    }
                }
                ...
                foreach($sets as $set) {
                    if($set === '' || !self::parse_set($times[$field_id], $type, $set)) $error = true;
                }
                if($error) {
                    $trace = debug_backtrace();
                    if(empty($times[$field_id]->index)) {
                        trigger_error("{$trace[1]['function']}: Skipping incorrect definition '".implode(' ', $fields)."' – due to unparseable definition-field value for $type ('".$fields[$field_id]."') on line $line of the definitions provided in {$trace[1]['file']} on line {$trace[1]['line']}.", E_USER_WARNING);
                        continue 2;
                    } else {
                        trigger_error("{$trace[1]['function']}: Ignored incorrect part(s) of the definition-field value for $type ('".$fields[$field_id]."' in definition '".implode(' ', $fields)."') on line $line of the definitions provided in {$trace[1]['file']} on line {$trace[1]['line']}.", E_USER_NOTICE);
                    }
                }
    ...
    static protected function parse_set(&$data, $type, $set) {
        ...
        if($set === '*') { # asterisk
            ...
            if($set >= $min && $set <= $max) $add = array_fill($set, 1, true);
        } elseif(preg_match('/^\d+\-\d+$/', $set)) { # digit range
            $set = explode('-', $set);
            if($set[0] < $min) $set[0] = $min;
            if($set[1] > $max) $set[1] = $max;
            if($set[0] <= $set[1]) {
                $add = array_fill($set[0], $set[1] - $set[0] + 1, true);
            }
        } elseif(preg_match('/^(\*|(\d+\-\d+))\/\d+$/', $set)) { # incremental range
            $set = explode('/', $set);
            if($set[0] === '*') $set[0] = array($min, $max);
            else {
                $set[0] = explode('-', $set[0]);
                if($set[0][0] < $min) $set[0][0] = $min;
                if($set[0][1] > $max) $set[0][1] = $max;
            }
            if($set[0][0] <= $set[0][1]) {
        ...
        if(isset($add)) {
            foreach($add as $key => $val) $data->index[$key] = 1;
            return true;
        }
        return false;
    }
    ...
    static protected function parse_time($time) {
        ...
        elseif(is_string($time) || is_numeric($time)) {
            if(preg_match('/^\d*$/', $time)) $time = (int)$time;
            else $time = @strtotime($time);
        }
        if(!is_int($time)) {
            $trace = debug_backtrace();
            trigger_error("{$trace[1]['function']}: Incorrect value given for \$time in {$trace[1]['file']} on line {$trace[1]['line']}. Using current time instead.", E_USER_NOTICE);
            $time = time();
        }
        return $time;
    }
     ...
     */
    static private function ly($year) {
        if($year % 100 === 0) {
