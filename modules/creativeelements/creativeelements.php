...
define('_CE_PS174P_', version_compare(_PS_VERSION_, '1.7.4', '>='));
...
class CreativeElements extends Module
{
    ...
    public function hookDisplayBackOfficeHeader($params)
    {
        ...
        switch ($controller) {
            ...
            case 'adminxippost':
                ...
                $id = (int) Tools::getValue('id_xipposts', $req ? $req[2] : 0);
                break;
            case 'adminstblog':
                $id_type = CE\UId::STBLOG_POST;
                $id = (int) Tools::getValue('id_st_blog', $req ? $req[2] : 0);
                break;
            case 'adminblogposts':
                $id_type = CE\UId::ADVANCEBLOG_POST;
                $id = (int) Tools::getValue('id_post', $req ? $req[2] : 0);
                break;
            case 'adminsimpleblogposts':
                if (_CE_PS16_) {
                    $id_type = CE\UId::SIMPLEBLOG_POST;
                    $id = (int) Tools::getValue('id_simpleblog_post', $req ? $req[2] : 0);
                }
                break;
            case 'adminmodules':
    ...
    public function hookOverrideLayoutTemplate($params)
    {
        ...
        switch ($front) {
            ...
            case 'xipblogsingle':
                ...
                if (!_CE_PS16_ && isset($tpl_vars['xipblogpost']->value['id_xipposts'])) {
                    ...
                    $desc = array('description' => &$tpl_vars['xipblogpost']->value['post_content']);
                } elseif (isset($controller->blogpost['id_xipposts'])) {
                    $id = $controller->blogpost['id_xipposts'];
                    $desc = array('description' => &$controller->blogpost['post_content']);
                }
                break;
            case 'stblogarticle':
                $model = 'StBlogClass';
                ...
                if (isset($tpl_vars['blog']->value['id'])) {
                    ...
                    $desc = array('description' => &$tpl_vars['blog']->value['content']);
                    break;
                }
                ...
                if (isset($blog->id)) {
                    $id = $blog->id;
                    $desc = array('description' => &$blog->content);
                }
                break;
            case 'advanceblogblog':
                $model = 'BlogPosts';

                if (isset($tpl_vars['postData']->value['id_post'])) {
                    $id = $tpl_vars['postData']->value['id_post'];
                    $desc = array('description' => &$tpl_vars['postData']->value['post_content']);
                }
                break;
            case 'prestablogblog':
                $model = 'NewsClass';
                ...
                if (isset($news->id)) {
                    ...
                    if (isset($tpl_vars['tpl_unique'])) {
                        $desc = array('description' => &$tpl_vars['tpl_unique']->value);
                    } else {
                        $desc = array('description' => &$news->content);
                    }
                }
                break;
            case 'ph_simpleblogsingle':
                ...
                if (_CE_PS16_ && isset($controller->SimpleBlogPost->id)) {
                    ...
                    $desc = array('description' => &$controller->SimpleBlogPost->content);
                }
                break;
        }
        ...
        $this->tplOverride = CE\apply_filters('template_include', '');
