...
FlipBoxHandler = function(self) { return self = {
	...
	init: function() {
		if (!elementor.$previewContents) {
			return elementor.on('preview:loaded', $.proxy(this, 'init'));
		}
		if (elementor.$previewContents.find('#ce-flip-box-edit-back').length) {
	...
	backStyle: function() {
		var wrapper = '.elementor-widget-flip-box.elementor-element-' + elementor.panel.currentView.content.currentView.model.id;
...
$(function onReady() {
	...
	var addPageCustomCss = function() {
		var customCSS = elementor.pageSettings.model.get('custom_css');

		if ( customCSS ) {
			customCSS = customCSS.replace(/selector/g, '.elementor-page-' + elementor.config.post_id);
			elementor.pageSettings.controlsCSS.elements.$stylesheetElement.append(customCSS);
		}
	};
	elementor.on('preview:loaded', addPageCustomCss);
	elementor.pageSettings.model.on('change', addPageCustomCss);
	...
	// init flip box widget
	ceFrontend.hooks.addAction('frontend/element_ready/flip-box.default', function($widget, $) {
		FlipBoxHandler.init();
	});
