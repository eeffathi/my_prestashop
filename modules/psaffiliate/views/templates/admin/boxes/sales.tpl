    ...
    {if isset($sales) && $sales && count($sales)}
        <table class="table">
            ...
            <tr>
                ...
                {/if}
                {if $mlm_enable}
                    <th><span class="title_box">{l s='Aff origin' mod='psaffiliate'}</span></th>
                {/if}
            ...
            {foreach $sales AS $key => $tr}
                <tr ondblclick="document.location = '{$link->getAdminLink('AdminPsaffiliateSales')|escape:'html':'UTF-8'}&amp;id_sale={$tr.id_sale|escape:'htmlall':'UTF-8'}&amp;updateaff_sales'">
                    ...
                    {if $mlm_enable}
                        <td>
                            {if $tr.id_affiliate_origin}
                                <a target="_blank"
                                   href="{$link->getAdminLink('AdminPsaffiliateAffiliates')|escape:'html':'UTF-8'}&amp;id_affiliate={$tr.id_affiliate_origin|escape:'htmlall':'UTF-8'}&amp;viewaff_affiliates">
                                    {$tr.affiliate_origin_name|escape:'htmlall':'UTF-8'}
                                </a>
                            {else}
                                --
                            {/if}
                    ...
                    {/if}
                    <td>{if $tr.commission}{Psaffiliate::displayPriceOverride($tr.commission)|escape:'html':'UTF-8'}{else}{l s='--' mod='psaffiliate'}{/if}</td>
                    <td>{if $tr.order_total}{displayPrice price=$tr.order_total|escape:'html':'UTF-8'}{else}{l s='--' mod='psaffiliate'}{/if}</td>
                    <td>
                        {if $tr.approved}
