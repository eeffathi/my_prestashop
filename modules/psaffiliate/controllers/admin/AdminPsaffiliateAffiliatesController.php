...
class AdminPsaffiliateAffiliatesController extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->fields_list = array(
            ...
            'per_click' => array(
                ...
                'width' => 'auto',
                'type' => 'text',
                'callback' => 'displayPriceOverrideHelperList',
                'callback_object' => 'Psaffiliate',
                'search' => true,
                'havingFilter' => false,
            ),
            ...
            'per_max_commission' => array(
                'title' => $this->l('maximum commission'),
                'align' => 'text-left',
                'width' => 'auto',
                'type' => 'text',
                'callback' => 'displayPriceOverrideHelperList',
        ...
        $this->_join = 'LEFT JOIN `'._DB_PREFIX_.'customer` c ON (c.`id_customer` = a.`id_customer`)
                        LEFT JOIN `'._DB_PREFIX_.'aff_affiliates` a2 ON (a.`id_parent_affiliate` = a2.`id_affiliate`)';
        $this->_select = '
            ...
            ROUND(IFNULL((SELECT cm.`value` FROM `'._DB_PREFIX_.'aff_commission` cm WHERE cm.`type`="max_commission" AND cm.`id_affiliate`=a.`id_affiliate` ORDER BY `date` DESC LIMIT 1), (SELECT cm.`value` FROM `'._DB_PREFIX_.'aff_commission` cm WHERE cm.`type`="max_commission" AND cm.`id_affiliate`="0" ORDER BY `date` DESC LIMIT 1)), 2) as `per_max_commission`,
            FORMAT(IFNULL(((SELECT IFNULL(SUM(`commission`), 0) FROM `'._DB_PREFIX_.'aff_tracking` t WHERE t.`id_affiliate`=a.`id_affiliate`)+(SELECT IFNULL(SUM(`commission`), 0) FROM `'._DB_PREFIX_.'aff_sales` s WHERE s.`id_affiliate`=a.`id_affiliate` AND s.`approved`="1")-(SELECT IFNULL(SUM(`amount`), 0) FROM `'._DB_PREFIX_.'aff_payments` p WHERE p.`id_affiliate`=a.`id_affiliate`)), 0), 2) as `balance`,
            IF(a.`id_parent_affiliate` = 0, "-", CONCAT("#", a.`id_parent_affiliate`, " - ", a2.firstname, " ", a2.lastname)) as `parent_affiliate`
        ';
        $this->shopLinkType = '';
    ...
    public function renderView()
    {
        ...
        if ($affiliate['id_parent_affiliate']) {
            $affiliate['parent_affiliate_name'] = Db::getInstance()->getValue('SELECT CONCAT ("#", `id_affiliate`, " - ", `firstname`, " ", `lastname`) as `name` FROM `'._DB_PREFIX_.'aff_affiliates` WHERE `id_affiliate` = "'.(int)$affiliate['id_parent_affiliate'].'"');
            $affiliate['parent_affiliate_link'] = $this->context->link->getAdminLink('AdminPsaffiliateAffiliates').'&id_affiliate='.(int)$affiliate['id_parent_affiliate'].'&viewaff_affiliates';
        }
        ...
        $this->context->smarty->assign('mlm_enable', (bool)AffConf::getConfig('mlm_enable'));
    ...
    public function renderForm()
    {
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->l('Parent affiliate'),
                    'name' => 'id_parent_affiliate',
                    'col' => '4',
                    'class' => 'ajaxselectpicker',
                    'options' => array(
                        'query' => Affiliate::getAffiliates(
                            ...
                            false,
                            false,
                            (isset($this->object) ? $this->object->id_parent_affiliate : 0)
                        ),
                        'id' => 'id_affiliate',
                        'name' => 'idandname',
                    ),
                ...
                array(
                    'type' => 'text',
                    'suffix' => $iso_code,
                    'label' => $this->l('Maximum Commission Amount'),
                    'name' => 'per_max_commission',
                    'col' => '4',
                    'autocomplete' => false,
                ),
            ),
