        ...
        if (Tools::getValue('action')) {
            ...
            if ($action == "generateChart") {
                ...
                if (!$chart) {
                    ...
                    $sql = "SELECT DATE_FORMAT(`date`, '%Y/%m/%d') `date`, COUNT(*) `count` FROM `"._DB_PREFIX_."aff_sales` WHERE ".$this->dateBetween('date').$extrasql_where." AND id_affiliate_origin = 0 GROUP BY YEAR(`date`), MONTH(`date`), DAY(`date`) ORDER BY `date` ASC";
                    $chart = Db::getInstance()->executeS($sql);
                    $chart_data_0 = $this->buildDays($chart);

                    $sql = "SELECT DATE_FORMAT(`date`, '%Y/%m/%d') `date`, SUM(o.`total_paid_tax_incl` / o.`conversion_rate`) as `count` FROM `"._DB_PREFIX_."aff_sales` s LEFT JOIN `"._DB_PREFIX_."orders` o ON (o.`id_order`=s.`id_order`) WHERE ".$this->dateBetween('date').$extrasql_where." AND id_affiliate_origin = 0 GROUP BY YEAR(`date`), MONTH(`date`), DAY(`date`) ORDER BY `date` ASC";
                    $chart = Db::getInstance()->executeS($sql);
                    $chart_data_1 = $this->buildDays($chart);
