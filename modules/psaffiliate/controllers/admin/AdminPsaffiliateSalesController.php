...
class AdminPsaffiliateSalesController extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $this->fields_list = array(
            ...
            'affiliate_origin_name' => array(
                'title' => $this->l('Affiliate origin'),
                'align' => 'text-left',
                'width' => 'auto',
                'search' => true,
                //'havingFilter' => true,
        ...
        $this->_join .= ' LEFT JOIN `'._DB_PREFIX_.'aff_affiliates` ao ON (a.`id_affiliate_origin` = ao.`id_affiliate`)';
        $this->_select = '
            ...
            CONCAT("#", ao.`id_affiliate`, " - ", IF(ao.`id_customer` <> 0, CONCAT(c.`firstname`, " ", c.`lastname`), CONCAT(ao.`firstname`, " ", ao.`lastname`))) as `affiliate_origin_name`
        ';
        $this->shopLinkType = '';
    ...
    public function renderForm()
    {
        ...
        $this->fields_form = array(
            ...
            'input' => array(
                ...
                array(
                    'type' => 'select',
                    'label' => $this->l('Affiliate origin'),
                    'name' => 'id_affiliate_origin',
                    'class' => 'ajaxselectpicker',
                    'desc' => $this->l('If the sale came from a multi-level affiliate, not by the affiliate above, this will be filled'),
                    'col' => '4',
                    'options' => array(
                        'query' => Affiliate::getAffiliates(
                            false,
                            ...
                            false,
                            false,
                            (isset($this->obj) ? $this->obj->id_affiliate_origin : 0)
                        ),
                        'id' => 'id_affiliate',
                        'name' => 'idandname',
                    ),
