...
class PsaffiliateMyaccountModuleFrontController extends ModuleFrontController
{
    ...
    public function displayTemplate()
    {
        if ($this->context->customer->isLogged()) {
            ...
            if ($this->module->isAffiliate() && !Tools::getValue('submitRegisterAffiliate')) {
                ...
                $able_to_see_cost = '';
                $affiliates_allowed_to_see_cost = AffConf::getConfig('show_cost_groups[]');
                if ((bool)AffConf::getConfig('show_cost') && Validate::isLoadedObject($customer) && is_array($affiliates_allowed_to_see_cost) && !in_array(0,$affiliates_allowed_to_see_cost)) {
                    $able_to_see_cost = (bool)in_array($customer->id_default_group, $affiliates_allowed_to_see_cost);
                }
                ...
                $this->context->smarty->assign('show_cost', $able_to_see_cost);
                $this->context->smarty->assign('show_percent', AffConf::getConfig('show_percent'));
                ...
                $this->context->smarty->assign(
                    'mlm_enable',
                    (bool)AffConf::getConfig('mlm_enable')
                );
                if ((bool)AffConf::getConfig('mlm_enable')) {
                    $this->context->smarty->assign(
                        'mlm_affiliates',
                        $this->module->getMlmAffiliates($affiliate->id)
                    );
                    $this->context->smarty->assign(
                        'mlm_rates',
                        $this->module->getMlmRates()
                    );
                    $this->context->smarty->assign(
                        'mlm_parent_info',
                        $this->module->getMlmParentInfo($affiliate->id)
                    );
                }
                ...
                $this->module->loadClasses('VoucherToShare');
                $this->context->smarty->assign('hasVouchersToShare', VoucherToShare::hasVoucherTemplates(true));
                $this->context->smarty->assign('voucher_to_share_enabled', AffConf::getConfig('vouchers_tracking'));
                $this->context->smarty->assign('vouchers_to_share', VoucherToShare::getAffiliateVouchersToShare($affiliate->id, $this->context->language->id));
                $this->context->smarty->assign('enable_affiliates_details', AffConf::getConfig('enable_affiliates_details'));
                $campaigns = $affiliate->getCampaigns(true);

                foreach ($campaigns as $key => $campaign) {
                    $campaigns[$key]['earnings_clicks_sales_sum'] = Psaffiliate::displayPriceOverride($campaign['total_earnings_clicks'] + $campaign['total_earnings_sales']);
                }
                ...
                if (Validate::isLoadedObject($customer)) {
                    ...
                    if (Tools::getValue('submitRegisterAffiliate')) {
                        ...
                        if (!sizeof($this->errors)) {
                            ...
                            if ($this->module->hasSessionAffiliate()) {
                                $affiliate->id_parent_affiliate = (int)$this->context->cookie->id_session_affiliate;
                            } elseif ($this->module->customerHasLifetimeAffiliate($customer->id)) {
                                $affiliate->id_parent_affiliate = (int)$this->module->customerHasLifetimeAffiliate($customer->id);
                            }
    ...
    public function getProductsWithCommisions($affiliate, $start, $limit, $count = false, $search = null)
    {
        ...
        $product_commisions = Db::getInstance()->executeS("
            ...
            on p.`id_product` = pl.`id_product` and pl.`id_lang` = ".(int)$this->context->language->id." and  pl.`id_shop` = ".(int)$this->context->shop->id."
            ...
            and pr.multiplier != 0 and cr.multiplier != 0) OR ((pr.id_product IS NULL OR cr.id_category IS NULL) and p.`active` = 1))
            {$like}
        ...
        array_walk($product_commisions, function (&$product) use ($affiliate) {
            ...
            $product['commission_rate'] = $this->module->getRatesForProduct(
                (int)$product['id_product'],
                (int)$affiliate->id
            )['rate_percent'];
            $product['cost'] = Tools::displayPrice($this->module->getWholesalePrice((int)$product['id_product']));
    ...
    public function getSellerSaleCommissions($seller, $start, $limit, $count = false)
    {
        $pr = _DB_PREFIX_;
        $select = !$count
            ? 'select s.*, ((o.`total_products_wt` - o.`total_discounts_tax_incl`) / o.`conversion_rate`) as `order_total`, o.`id_currency` as `order_currency`, CONCAT("#", ao.`id_affiliate`, " - ", IF(ao.`id_customer` <> 0, CONCAT(c.`firstname`, " ", c.`lastname`), CONCAT(ao.`firstname`, " ", ao.`lastname`))) as `affiliate_origin_name`'
        ...
        $sales = Db::getInstance()->executeS("
            ...
            left join `{$pr}aff_affiliates` ao
            on (s.`id_affiliate_origin` = ao.`id_affiliate`)
            left join `{$pr}customer` c
            on c.`id_customer` = ao.`id_affiliate`
        ...
        array_walk($sales, function (&$sale) {
            $sale['commission'] = Psaffiliate::displayPriceOverride(Tools::convertPrice($sale['commission']));
