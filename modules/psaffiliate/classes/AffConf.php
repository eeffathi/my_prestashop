...
class AffConf
{
        ...
        return array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'radio',
                        'label' => $this->l('How should the commissions be calculated?'),
                        ...
                        'values' => array(
                            array(
                                'id' => 'c_prod',
                                'value' => 1,
                                'label' => $this->l('Based on product price'),
                            ),
                            array(
                                'id' => 'c_diff',
                                'value' => 2,
                                'label' => $this->l('Based on the difference between wholesale and product price'),
                            ),
                            array(
                                'id' => 'c_wholesale',
                                'value' => 3,
                                'label' => $this->l('As difference between wholesale and product price'),
                            ),
                        ),
                        'desc' => $this->l('This changes how commissions are issued.'),
                    ),
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Should the affiliates see the product cost?'),
                        'desc' => $this->l(''),
                        'name' => 'show_cost',
                        'class' => 't',
                        'is_bool' => true,
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Should the affiliates see the commission percentage?'),
                        'desc' => $this->l(''),
                        'name' => 'show_percent',
                        'class' => 't',
                        'is_bool' => true,
                    ...
                    array(
                        'col' => 5,
                        'type' => 'select',
                        'desc' => $this->l('If you wish to choose which types of affiliates see the product cost, select one of the following. In the event that no groups are chosen, then all of the affiliates will see the product cost.'),
                        'name' => 'show_cost_groups[]',
                        'label' => $this->l('Choose which affiliate sees the product cost.'),
                        'multiple' => true,
                        'options' => array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Sales commissions only for new customers'),
                        'desc' => $this->l('Your affiliates will only receive commissions from sales if the sale is made by a new customer. New customer means that there was no order placed from that account before.'),
                        'name' => 'commissions_only_new_customers',
                        'required' => false,
                        'class' => 't',
                        'is_bool' => true,
                        'values' => array(
                    ...
                    array(
                        'col' => '4',
                        'type' => 'switch',
                        'label' => $this->l('Override affiliate with Voucher to Share'),
                        'desc' => array(
                            $this->l('Let\'s say affiliate A brings you a customer. If that customer then enters affiliate B\'s Voucher at checkout, one of the following happens:'),
                            $this->l('1) if override enabled, affiliate B will get the commission'),
                            $this->l('2) if override disabled, affiliate A will get the commission'),
                        ),
                        'name' => 'override_vts_affiliate',
                        'required' => false,
                        'class' => 't',
                        'is_bool' => true,
                        'values' => array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Enable affiliates details'),
                        'name' => 'enable_affiliates_details',
                        'required' => false,
                        'class' => 't',
                        'is_bool' => true,
                        'values' => array(
                        ...
                        'desc' => $this->l('Enable affiliates you brought details pop-up.'),
                    ),
                    array(
                        'type' => 'switch',
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Show commissions as custom currency'),
                        'desc' => $this->l('Describe the commissions as something different than currency.'),
                    ...
                    array(
                        'col' => 5,
                        'type' => 'text',
                        'desc' => $this->l('How should the alternative payment method be named? ex. points'),
                        'name' => 'commissions_name',
                        'label' => $this->l('Your new custom currency name'),
                        'lang' => true,
                    ),
                ),
                'submit' => array(
            ...
            foreach ($langs as $lang) {
                if (!isset($sql_lang_array[$sql_lang_row['name']][(int)$lang['id_lang']])) {
                    if (isset($sql_lang_array[$sql_lang_row['name']][$id_default_lang])) {
                        ...
                        $array_values = array_values($sql_lang_array[$sql_lang_row['name']]);
                        $sql_lang_array[$sql_lang_row['name']][(int)$lang['id_lang']] = array_shift($array_values);
                    }
                }
            }
        }
    ...
    public static function updateConfig($name, $value)
    {
        ...
        return (bool)Db::getInstance()->insert('aff_configuration', $data_to_insert, true, false, Db::REPLACE);
    }
