...
class Affiliate extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_parent_affiliate' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'copy_post' => false),
            ...
            'password' => array('type' => self::TYPE_STRING, 'validate' => 'isPasswd', 'size' => 32),
    ...
    public function __construct($id = null)
    {
        ...
        if ($id) {
            ...
            $this->earnings_formatted = Psaffiliate::displayPriceOverride($this->earnings);
            $this->earnings_total = $this->getEarnings(true, true);
            $this->earnings_total_formatted = Psaffiliate::displayPriceOverride($this->earnings_total);
            $this->payments = $this->getPayments();
            $this->payments_formatted = Psaffiliate::displayPriceOverride($this->payments);
            $this->pending_payments = $this->getPendingPayments();
            $this->pending_payments_formatted = Psaffiliate::displayPriceOverride($this->pending_payments);
            $this->balance = (float)$this->earnings_total - (float)$this->payments - (float)$this->pending_payments;
            $this->balance_formatted = Psaffiliate::displayPriceOverride($this->balance);
        }
        $this->getRates();
        $this->getMetas();
    }
    ...
    public function add($auto_date = true, $null_values = false)
    {
        ...
        if (isset($controller_name) && Tools::substr(Tools::strtolower($controller_name), 0, 5) != "admin") {
            $rates = $this->moduleObj->getGeneralRates();
            if ($rates) {
                foreach ($rates as $type => $rate) {
                    $_POST["per_".$type] = (float)$rate;
                }
            }
        }
    ...
    public function update($null_values = false)
    {
        ...
        $rates_to_check = array(
            ...
            'per_max_commission' => 'max_commission'
        );
    ...
    public function getRates($type = false)
    {
        $db = Db::getInstance();
        if (!$type) {
            $type = array('click', 'unique_click', 'sale', 'sale_percent', 'max_commission');
    ...
    public static function getCustomersList($only_active = false, $id_customer = null)
    {
        $sql = 'SELECT `id_customer`, `email`, `firstname`, `lastname`, CONCAT(`firstname`, " ", `lastname`) as `name`, CONCAT("#", `id_customer`, " - ", `firstname`, " ", `lastname`, " - ", `email`) as `idandname`
            ...
            (!is_null($id_customer) ? ' AND `id_customer` = "'.(int)$id_customer.'"' : '').'
                ORDER BY `id_customer` ASC LIMIT 50';
    ...
    public static function getAffiliates(
        ...
        if ($details) {
            ...
            $sql .= ', (SELECT COUNT(*) FROM `'._DB_PREFIX_.'aff_sales` asa WHERE asa.`id_affiliate`=a.`id_affiliate` AND asa.`id_affiliate_origin` = 0) as `sales`';
            $sql .= ', (IFNULL((SELECT ROUND(SUM(`commission`),2) FROM `'._DB_PREFIX_.'aff_tracking` at WHERE at.`id_affiliate`=a.`id_affiliate` AND `commission` > 0), 0) + IFNULL((SELECT ROUND(SUM(`commission`),2) FROM `'._DB_PREFIX_.'aff_sales` asa WHERE asa.`id_affiliate`=a.`id_affiliate` AND `commission` > 0), 0)) as `overall_commission`';
        }
