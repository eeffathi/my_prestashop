...
class Sale extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'id_affiliate_origin' => array('type' => self::TYPE_INT, 'validate' => 'isUnsignedInt', 'copy_post' => false),
    ...
    {
        $sql = "SELECT s.*, ((o.`total_products_wt` - o.`total_discounts_tax_incl`) / o.`conversion_rate`) as `order_total`";
        if (!$id_affiliate) {
        ...
        $sql .= ', CONCAT("#", ao.`id_affiliate`, " - ", IF(ao.`id_customer` <> 0, CONCAT(co.`firstname`, " ", co.`lastname`), CONCAT(ao.`firstname`, " ", ao.`lastname`))) as `affiliate_origin_name`';

        $sql .= " FROM `"._DB_PREFIX_."aff_sales` s LEFT JOIN `"._DB_PREFIX_."orders` o ON (s.`id_order` = o.`id_order`)";
        $sql .= ' LEFT JOIN `'._DB_PREFIX_.'aff_affiliates` ao ON (s.`id_affiliate_origin` = ao.`id_affiliate`)';
        $sql .= ' LEFT JOIN `'._DB_PREFIX_.'customer` co ON (co.`id_customer` = ao.`id_customer`)';
        if ($id_affiliate) {
            $sql .= " WHERE s.`id_affiliate`='".(int)$id_affiliate."'";
        } else {
