...
function upgrade_module_1_3_92()
{
    ...
    if (is_array($list_fields)) {
        ...
        if (!in_array('select_for', $list_fields) && $res) {
            $res = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'seosaproductlabels`
                ADD `select_for` INT(11) NOT NULL');
        }
    }
    return $res && Db::getInstance()->execute('CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'seosaproductlabels_supplier` (
