...
function upgrade_module_1_4_6()
{
    ...
    if (is_array($list_fields)) {
        ...
        if (!in_array('quantity', $list_fields) && $res) {
            $res = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'seosaproductlabels`
                ADD `quantity` VARCHAR(32) NOT NULL');
        }
        if (!in_array('quantity_max', $list_fields) && $res) {
            $res = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'seosaproductlabels`
                ADD `quantity_max` VARCHAR(32) NOT NULL');
        }
    }
    return $res;
}
