...
function upgrade_module_1_4_4()
{
    ...
    if (is_array($list_fields)) {
        ...
        if (!in_array('mini_price', $list_fields) && $res) {
            $res = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'seosaproductlabels`
                ADD `mini_price` decimal(10,2) NOT NULL DEFAULT "0.00"');
        }
        if (!in_array('max_price', $list_fields) && $res) {
            $res = Db::getInstance()->execute('ALTER TABLE `'._DB_PREFIX_.'seosaproductlabels`
                ADD `max_price` decimal(10,2) NOT NULL DEFAULT "0.00"');
        }
    }
    return $res;
}
