...
{foreach from=$seosa_product_labels item=product_label}
  ...
  {if $product_label['max_price'] > $product_label['price'] && $product_label['mini_price'] < $product_label['price'] || $product_label['max_price'] == 0 &&  $product_label['mini_price'] == 0}
    <div class="seosa_product_label _product_page {$product_label['position']|escape:'quotes':'UTF-8'}"
            {if $product_label['label_type'] == 'image' && $product_label['image_css']}
    style="{$product_label['image_css']|escape:'quotes':'UTF-8'}"{elseif $product_label['label_type'] == 'text' && $product_label['text']} style="width: auto; height: auto;"{/if}>
         {*style="{if $product_label['label_type'] == 'text' && $product_label['text_css']}{$product_label['text_css']|escape:'quotes':'UTF-8'}{elseif $product_label['label_type'] == 'image' && $product_label['image_css']}{$product_label['image_css']|escape:'quotes':'UTF-8'}{/if}">*}
      <p id="mini_price" style="display: none" data-min="{$product_label['mini_price']}"></p>
      <p id="max_price" style="display: none"  data-max="{$product_label['max_price']}"></p>
        {if $product_label['url']}
        <a target="_blank" href="{$product_label['url']|escape:'quotes':'UTF-8'}">
        {/if}
        {if $product_label['label_type'] == 'image'}
            <img src="{$product_label['image_url']|escape:'quotes':'UTF-8'}"{if $product_label['image_css']} style="{$product_label['image_css']|escape:'quotes':'UTF-8'}"{/if} alt="{$product_label['name']|escape:'quotes':'UTF-8'}"/>
        ...
        {if $product_label['hint']}
          ...
          <style>
            ...
            {*.seosa_label_hint_{$product_label['id_product_label']|intval}_{$product_label['id_product']|intval}:after {*}
              {*border-{$product_label['fix_hint_position']|escape:'html':'UTF-8'}: solid {if $product_label['hint_background']}{$product_label['hint_background']|escape:'html':'UTF-8'}{else}#000000{/if} 10px;*}
