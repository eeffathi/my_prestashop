...
<script type="text/javascript">
	...
	function markBlockedFilters() {
		var $template = $('.af_template.open'),
			...
			templateController = $template.data('controller');
		$filters.each(function() {
			$('.filter-group-item[data-key="'+$(this).data('key')+'"]').addClass('blocked');
		});
		if (templateController == 'manufacturer' || templateController == 'supplier') {
			$('.filter-group-item[data-key="'+templateController.charAt(0)+'"]').addClass('blocked');
		}
		updateTotals(false);
	}
