...
$(document).ready(function() {
	...
	$.extend(af, {
		...
		instantSettings: function() {
			var $instantCheckbox = $('.instant-settings');
			$instantCheckbox.on('change', function() {
				var checked = $(this).prop('checked');
				$('.panel-footer.multiple-settings').toggleClass('hidden', checked);
				af.storage.save('instantSettings', checked ? 1 : 0);
			});
			if (af.storage.get('instantSettings') > 0) {
				$instantCheckbox.prop('checked', true).change();
			}
			$('.settings_form').on('change', 'input, select', function() {
				if ($instantCheckbox.is(':checked')) {
					$(this).closest('.tab-pane').find('.saveMultipleSettings').click();
					if ($(this).attr('name').startsWith('merged_')) {
						af.merged.toggleMenuItem($(this).attr('name').split('merged_')[1].slice(0, -1));
					}
				}
			});
			af.toggleInstansSettings();
		},
		toggleInstansSettings: function() {
			...
			response = function(r) {
				if ('errors' in r) {
					...
					if ($btn.hasClass('indexation')) {
						if (!$('.instant-settings').is(':checked')) {
